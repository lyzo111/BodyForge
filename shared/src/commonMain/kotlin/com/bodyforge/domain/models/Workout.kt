...
data class Workout(
    ...
    // Elapsed active time. Pauses from finishing and later resuming a workout are excluded by
    // shifting startedAt forward on resume (see SharedWorkoutState.resumeWorkout), so this stays a
    // plain elapsed-time calculation.
    val durationMinutes: Long? get() = finishedAt?.let {
        ((it.epochSeconds - startedAt.epochSeconds) / 60).coerceAtLeast(0)
    }
